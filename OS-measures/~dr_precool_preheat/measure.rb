...
# start the measure
class DRPreCoolingHeatingLgOffice < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
    ...
    if auto_date
      ...
      # if ashraeClimateZone == '2A' || ashraeClimateZone == '2B' || ashraeClimateZone == '3A' || ashraeClimateZone == '3B' || ashraeClimateZone == '3C '|| ashraeClimateZone == '4A' 
    ...
    winterEndDate1 = OpenStudio::Date.new(winterEndMonth1, 31, 2006)

    winterStartMonth2 = OpenStudio::MonthOfYear.new(winter_start_month2)
    winterEndMonth2 = OpenStudio::MonthOfYear.new(winter_end_month2)
    winterStartDate2 = OpenStudio::Date.new(winterStartMonth2, 1, 2006)
    winterEndDate2 = OpenStudio::Date.new(winterEndMonth2, 31, 2006)
      ...
      if !v.to_ScheduleRuleset.empty?
        ...
        if rules.length > 0
          ...
          rules.each do |rule|
            ...
            ##### specific to LargeHotel since cooling setp adjustment lower than the heating setpoint
            summer_rule = rule
            if (summer_rule.startDate().get == summerStartDate) && (summer_rule.endDate().get == summerEndDate) 
              summer_day = summer_rule.daySchedule
              day_time_vector = summer_day.times
              day_value_vector = summer_day.values
              summer_day.clearValues
              #summer_day = compareUpdateDaySchedule(summer_day, day_time_vector, day_value_vector, final_clg_sch_set_times, final_clg_sch_set_values)
              summer_day = createDaySchedule(summer_day, day_time_vector, day_value_vector, shift_time1, shift_time2, cooling_adjustment_ip)
              final_htg_sch_set_values << summer_day.values

            end
            ################# end modification for specific LargeHotel ######################
...
DRPreCoolingHeatingLgOffice.new.registerWithApplication
