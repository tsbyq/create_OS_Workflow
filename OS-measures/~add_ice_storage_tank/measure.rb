...
# start the measure
class AddIceStorageTank < OpenStudio::Measure::ModelMeasure

  # human readable name
  def name
    # Measure name should be the title case of the class name.
    return 'Add Ice Storage Tank'
  end

  # human readable description
  def description
    return 'This measure adds an ice storage tank to a chilled water loop for the purpose of thermal energy storage.'
  end
  ...
  def arguments(model)
    ...
    objective.setDefaultValue('Full Storage')
    args << objective
    ...
    chillers = chillers + model.getChillerAbsorptions
    chillers = chillers + model.getChillerAbsorptionIndirects
    ...
    chiller_resize_factor.setDefaultValue('1.0')
    ...
    all_scheds.each do |sched|
	  ...
	  if !sched.scheduleTypeLimits.empty?
    ...
    charge_start.setDefaultValue('00:00')
    args << charge_start
    ...
    charge_end.setDefaultValue('06:00')
    args << charge_end
    ...
    alt_periods.setDefaultValue(true)
    args << alt_periods        
    ...
    if auto_date
      ...
      if alt_periods
        if ashraeClimateZone == '3A'
          ...
          discharge_start = '14:00'
          discharge_end = '17:00'
        elsif ashraeClimateZone == '6A'
          discharge_start = '13:00'
          discharge_end = '16:00'
        end
      else
  ...
  cond_loop = ""
    ...
    if !ctes_loop.components.include?(ctes_chiller)
    ...
    if ds > de
      runner.registerWarning('Discharge start time is later than discharge ' \
    ...
    runner.registerInfo("Chiller '#{selected_chiller}' on Loop '#{selected_loop}' was selected for the addition " \
                        "of a #{storage_capacity.round(2)} GJ (#{(storage_capacity/12660670.23144e-9).round(0)} " \
    ...
    if ctes_loop.fluidType == 'Water'
      ...
      ctes_loop.setGlycolConcentration(25)  ## previously 25%
    ...
    if new
      ...
      ruleset_name = "#{ctes.name.to_s} Setpoint Schedule (New)"
      ...
      ruleset_name = "#{ctes_chiller.name.to_s} Setpoint Schedule (New)"
      ...
      if new
        ruleset_name = "#{ctes_loop.name.to_s} Setpoint Schedule (New)"
    ...
    ctes_sp_mgr.setName("#{ctes.name.to_s} Setpoint Manager")
    ...
    chill_sp_mgr.setName("#{ctes_chiller.name.to_s} Setpoint Manager")
    ...
    if new && !old
		...
		loop_sp_mgrs = loop_sp_node.setpointManagers
		loop_sp_mgrs.each do |spm|
			if ['Temperature','MinimumTemperature','MaximumTemperature'].include?(spm.controlVariable)
				spm.disconnect
				runner.registerInfo("Selected loop temperature setpoint manager '#{spm.name.to_s}' " \
									"with control variable '#{spm.controlVariable.to_s}' was disconnected.")
			end
		end

		loop_sp_mgr = OpenStudio::Model::SetpointManagerScheduled.new(model, loop_sch_new)
		loop_sp_mgr.addToNode(loop_sp_node)
		loop_sp_mgr.setName("#{ctes_loop.name.to_s} Setpoint Manager (New)")

		demand_sp_mgr = loop_sp_mgr.clone.to_SetpointManagerScheduled.get
		demand_sp_mgr.addToNode(ctes_loop.demandInletNode)
		demand_sp_mgr.setName("#{ctes_loop.name.to_s} Demand Side Setpoint Manager (New)")
	end
...
AddIceStorageTank.new.registerWithApplication
